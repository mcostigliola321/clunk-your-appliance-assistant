...
At the model level, tier totals are 55 **Purchase-ready**, 108 **Guided checks**, and zero **Verified part unavailable**. At the model × symptom level, the 163 identities resolve to 557 packs: the same 55 purchase-ready combinations plus 502 guided combinations. The catalog expansion adds 113 entries through 50 validated family profiles; later exact-part overlays upgrade only separately evidenced complete engineering revisions. Exact-part data lives on the model row—not the shared profile—so no neighboring revision or symptom inherits a part or Shopify query.
...
- Thirty later exact revisions became purchase-ready only after a complete-code-to-SKU mapping was found on a manufacturer or authorized-parts chain: 21 refrigerator filters, six Bosch dishwasher drain pumps, and three Samsung dishwasher drain pumps. Ambiguous Samsung/LG pump pages and refrigerator filter pages with multiple candidates were not promoted.
