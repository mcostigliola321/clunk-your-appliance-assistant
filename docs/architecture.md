...
- `src/data/symptomCatalog.ts` defines the visible problem families, category scope, and mature-versus-limited presentation. `src/data/symptomCoverageExpansion.json` records 91 exact door-closure model × symptom rows with category, topology, primary source, applicability, feature exceptions, safe checks, professional boundaries, and a 2026-08-28 verification date; `src/data/symptomCoverageExpansion.ts` validates and materializes them. Model identity and symptom coverage remain separate claims.
...
The catalog currently contains 163 identities and 266 repair packs: 25 purchase-ready and 241 guided checks. The homepage primary layer exposes only the seven mature category/problem cohorts (56, 36, 33, 20, 33, 41, and 35 checked models respectively). The twelve one-model supplemental routes remain in the protocol and search contracts but are presented in a secondary limited-pilot disclosure.
