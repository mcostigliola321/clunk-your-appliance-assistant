...
- `src/data/applianceCatalog.ts` composes the bounded 163-model catalog: 50 milestone entries plus 113 schema-validated expansion entries. It keeps explicit capability tiers, official source references, category/profile metadata, exact part mappings, and dated Shopify UCP query descriptors. `src/data/catalogExpansion.ts` validates revision isolation and materializes the compact expansion data. `src/data/purchaseCoverageExpansion.ts` then overlays only 33 separately reviewed exact revisions—21 refrigerator filter routes, nine dishwasher drain routes, one washer drain route, and two dryer door-strike routes—in `purchaseCoverageExpansion.json`; it rejects an unknown or sibling revision, a non-primary compatibility host, a checks profile outside the reviewed route, a wrong-category component, an unproved SKU, or a zero-offer commerce descriptor before repair-pack generation.
...
The catalog currently contains 163 identities and 557 repair packs: 58 purchase-ready and 499 guided checks. The purchase-ready identities are 9 washers, 13 dishwashers, 9 dryers, and 27 refrigerators; all other model × symptom records remain guided or unsupported as explicitly recorded. The 12 formerly one-model routes have explicit multi-brand breadth: 39 checked washers per route, 21 dishwashers per route, 19 electric dryers per route, and 22 refrigerators per route. The homepage shows at most four common problems and places a fifth checked route under **More problems**; this layout never grants coverage to an unlisted model × symptom pair.
