...
- `src/webmcp/contracts.ts` is the bounded public tool catalog used by registration, tests, eval fixtures, and the visible inspector. It declares each input schema; catalog- and step-derived IDs are bounded strings that must be copied from earlier results, not duplicated as large enums. Search may filter by problem and selection may bind one when search did not; `start_diagnosis` takes no duplicate problem argument and starts the already-selected pair.
- `src/webmcp/toolOutputs.ts` projects engine state into eight distinct structured-result contracts. Every tool returns phase, handoff, and `nextTools`; only `get_repair_state` returns the full snapshot, while each action returns the catalog, selection, check, focus, outcome, or escalation it owns.
- `src/webmcp/registerTools.ts` contains eight literal imperative registrations exposed together on page load, tool-specific structured results, feature detection, and `AbortController` lifecycle cleanup.
...
Normal buttons, labeled one-click example fixtures, the visible manual inspector, and browser-agent calls all invoke `executeRepairTool`. Mutating actions return the next state and a visible activity event. `get_repair_state` is genuinely read-only and returns the complete current-task structured content at the WebMCP boundary. Other tools return narrower, schema-validated projections rather than repeating that full state. Example fixtures are sequences of existing public actions, not pre-rendered outcomes. A rejected call is also logged but cannot advance progress.
...
The browser build is static HTML, CSS, JavaScript, JSON, and local font files. It has no database, authentication, payment handling, model SDK, or secret. Diagnosis only makes an optional keyless Shopify catalog lookup on an exact outcome. Results use `no-store`, are not persisted, and degrade to an honest retry/no-offer state without weakening the source-backed compatibility result.

The secondary **One guide. Two ways to use it.** explanation also offers the submitted 2:30 demo (`9eQbt7B8rQs`). `DemoVideo` creates a YouTube privacy-enhanced iframe only after an explicit click, with autoplay disabled and a direct YouTube fallback. Closing the video or its parent explanation unmounts the iframe and stops playback. It does not touch repair state or register tools. The host CSP permits only `https://www.youtube-nocookie.com` in `frame-src`. The player follows [YouTube's embed parameters](https://developers.google.com/youtube/player_parameters), preserving a 200px minimum height on small screens and 16:9 sizing where space allows.
