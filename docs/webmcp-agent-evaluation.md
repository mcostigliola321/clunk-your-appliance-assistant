...
On 2026-09-02, a fresh Codex in-app browser client check against the contract-refined live build discovered all eight in-page tools and directly called `get_repair_state`, `search_supported_appliances`, `select_appliance`, and `start_diagnosis`. The visible page advanced from the four-appliance home to the GE `GTD42EASJ2WW` safety step, and the response changed `nextTools` to include `record_observation` while excluding premature part lookup. The client correctly refused to manufacture the scripted physical safety observation without a person. This is useful client/tool and human-boundary evidence, but it is not counted as a completed autonomous natural-language-agent case. The connected Chrome profile did not have WebMCP testing enabled and is not counted as passing.
