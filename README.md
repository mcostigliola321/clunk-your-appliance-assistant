...
All eight tools are registered on page load so browser agents and directories can discover the complete workflow. Every result has a tool-specific structured-output schema plus a compact shared phase, handoff, and `nextTools` envelope: search returns catalog matches, selection returns the selected model/problem, diagnosis returns the current check, component focus returns only the focused component, and terminal tools return only their outcome or escalation. `get_repair_state` remains the deliberate full snapshot. IDs that depend on catalog or repair state must come from prior tool output instead of a copied catalog-sized enum, while the engine still rejects invented IDs, out-of-order observations, unsupported model/problem pairs, and post-terminal advancement. Inputs use bounded schemas with `additionalProperties: false`, registration lifecycle is managed with an `AbortController`, and the same public action layer serves the UI, sample guides, inspector, and browser-agent calls.
