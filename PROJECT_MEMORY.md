...
Last reconciled: **2026-08-29** during the unpushed purchase-coverage goal on `codex/expand-purchase-ready-coverage`, started from and still tracking `origin/main` at `98b90bb2a387d66e779cebf40e3eac000960a51f`. Purchase-coverage implementation commits are `403b9ea`, `579fcfe`, and `fc9611a`, followed by this memory reconciliation at the branch tip; the clean working branch is four commits ahead with 58 purchase-ready models, while `main` still has 25 until this work is reviewed and explicitly merged. Confirm moving remote and deployment state before relying on this handoff; production publication has not been verified.
...
- Model identity and symptom coverage remain many-to-many. The branch resolves to the same 557 model × symptom packs: 58 purchase-ready and 499 guided-check combinations. Category purchase-ready counts are 9 washers, 13 dishwashers, 9 electric dryers, and 27 refrigerators.
- `src/data/purchaseCoverageExpansion.json` and its runtime validator add 33 separately reviewed exact revisions: 21 refrigerator filter paths, nine dishwasher drain-pump paths, `WT7400CW.ABWEUUS` → `AHA75673404`, and `DLE6100W.ABWETUS`/`DLE7000W.ABWETUS` → `4026EL3007C`. Every row binds one exact code to one primary/authorized compatibility chain; shared part profiles cannot grant model coverage. Non-default symptoms remain guided-only.
- Fresh Shopify Global Catalog checks retained at least five exact-number offers for the nine earlier overlay SKUs, eight for `AHA75673404`, and ten for `4026EL3007C`. Shopify and public storefront feeds remain offer/candidate discovery only.
- The remaining evidence queue is 105 checks-only models: 31 have a verified complete code but no single proven matching part, while 74 remain family-only. LG washer revisions with suffix mismatches or multiple drain pumps remain guided. `DLEX4000W.ABWEUUS` and `DLEX6500B.ABLEECI` prove only the cabinet catch, not the observed door-side strike. GE `GFE28GYNFS`/`GNE29GYNFS` and KitchenAid `KRFF305ESS00` expose multiple filter candidates; Samsung `DW80CG5450SR/AA` lacks an equally exact authorized pump mapping. None was promoted.
...
The 2026-08-29 purchase-coverage branch release gate passed after the LG cohort: TypeScript, ESLint, all 98 unit/integration/WebMCP tests, production build, all 50 desktop/mobile Playwright cases, and `npm audit --audit-level=moderate` with zero vulnerabilities. A bounded visual review covered the desktop and 320px homepages plus the new exact LG dryer result, with earlier coverage retained for dishwasher counts, unsupported model × symptom state, professional safety stop, and exact commerce. The production build retains Vite's existing advisory for a JavaScript chunk above 500 kB; the built JavaScript is about 171 kB gzip.
